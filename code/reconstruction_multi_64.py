...
ml = MyLogger("reconstruction_multiprotocol_64")

reconstruct_until_timestep = 18101

md.set_collection('intra_mappings_64')
...
md.set_collection('inter_mappings_64')
documents = md.collection.find()
inter_df = pd.DataFrame(documents)

md.set_collection('reconstruction_baseline_64')
documents = md.collection.find()
...
for index, inter_row in inter_df.iterrows():
    ...
    if max_last_timestep > reconstruct_until_timestep:
        max_last_timestep = reconstruct_until_timestep
...
# difference = reconstruct_until_timestep - multi_protocol_df['start_timestep']

# # Replace ideal_duration with the calculated difference where applicable
# multi_protocol_df['ideal_duration'] = multi_protocol_df.apply(lambda row: difference[row.name] if row['ideal_duration'] > difference[row.name] else row['ideal_duration'], axis=1)
...
# print(multi_protocol_df.to_string())
multi_protocol_df.to_csv('csv/multi_protocol_64.csv', index=False)
md.db['reconstruction_multiproto_64'].drop()
md.db['reconstruction_multiproto_64'].insert_many(multi_data)