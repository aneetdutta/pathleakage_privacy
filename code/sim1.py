...
if len(sys.argv) < 3:
    print("Usage: python script_name.py <number_of_users>")
    print("Usage: python script_name.py <number_of_sniffers>")
    sys.exit(1)
...
class User:
    def __init__(self, user_id, bluetooth_id, wifi_id, lte_id, location, max_step_size=0.1):
    ...
    def randomize_identifiers(self):
        ...
        if self.identifier_counter % self.interval_lte==0:
            ...
            self.lte_id = ''.join(random.choices(string.ascii_uppercase + string.digits, k=12))
            print(self.lte_id)
...
detected_users=[]
class Sniffer:
    def __init__(self,sniffer_identifier,location, bluetooth_range, wifi_range, lte_range):
    ...
    def detect_devices(self,devices,timestep):
        ...
        for device in devices:
            for i in range(0,3):
                ...
                distance = ((self.location[0] - device[i].location[0]) ** 2 + (self.location[1] - device[i].location[1]) ** 2) ** 0.5
                 #print(distance)
                 #print(isinstance(device[i], Bluetooth))
                if isinstance(device[i], Bluetooth) and distance <= self.bluetooth_range:
                    ...
                    sniffed_devices[s]=[timestep,protocol,device[i].device_id,device[i].location]
                    detected_devices.append({"timestep":timestep,"protocol":protocol,"bluetooth_id":device[i].device_id,"location":device[i].location})
                    ...
                    s=str(timestep)+str(device[i].device_id)
                    sniffed_devices[s]=[timestep,protocol,device[i].device_id,device[i].location]
                    detected_devices.append({"timestep":timestep,"protocol":protocol,"wifi_id":device[i].device_id,"location":device[i].location})
                    ...
                    s=str(timestep)+str(device[i].device_id)
                    sniffed_devices[s]=[timestep,protocol,device[i].device_id,device[i].location]
                    detected_devices.append({"timestep":timestep,"protocol":protocol,"lte_id":device[i].device_id,"location":device[i].location})
                     #print(sniffed_devices)
    def detect_users(self,user,timestep):
        
        distance = ((self.location[0] - user.location[0]) ** 2 + (self.location[1] - user.location[1]) ** 2) ** 0.5
        if distance<=self.bluetooth_range:
            protocol='Bluetooth'
            detected_users.append({"timestep":timestep,"sniffer_id":self.id,"sniffer_location":self.location,"protocol":protocol,"bluetooth_id":user.bluetooth_id,"location":user.location})
        if distance<=self.wifi_range:
            protocol='WiFi'
            detected_users.append({"timestep":timestep,"sniffer_id":self.id,"sniffer_location":self.location,"protocol":protocol,"WiFi_id":user.wifi_id,"location":user.location})  
        if distance<=self.lte_range:
            protocol='LTE'
            detected_users.append({"timestep":timestep,"sniffer_id":self.id,"sniffer_location":self.location,"protocol":protocol,"lte_id":user.lte_id,"location":user.location})    
...
for timestep in range(500):
    for user in users:
        ...
        user_data.append({"timestep":timestep,"user_id": user.user_id, "location": user.location, "bluetooth_id": user.bluetooth_id, "wifi_id":user.wifi_id,"lte_id":user.lte_id })
        print("---")
