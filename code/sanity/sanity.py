...
for index, row in user_data.iterrows():
    ...
    for inter_id, inter_ids in inter_data.items():
        ...
        if ENABLE_LTE:
            p1_lte = {inter_id}.intersection(lte_ids)
            p2_lte = inter_ids.intersection(lte_ids)
        else:
            p1_lte = set()
            p2_lte = set()
            
        if ENABLE_WIFI:
            p1_wifi = {inter_id}.intersection(wifi_ids)
            p2_wifi = inter_ids.intersection(wifi_ids)
        else:
            p1_wifi = set()
            p2_wifi = set()
            
        if ENABLE_BLUETOOTH:
            p1_ble = {inter_id}.intersection(ble_ids)
            p2_ble = inter_ids.intersection(ble_ids)
        else:
            p1_ble = set()
            p2_ble = set()
        ...
        if p1_lte and (p2_wifi or p2_ble):
        ...
        if p1_wifi and (p2_lte or p2_ble):
        ...
        if p1_ble and (p2_lte or p2_wifi):
...
for index, row in user_data.iterrows():
    ...
    # if not ENABLE_BLUETOOTH:
    if ENABLE_WIFI and not ENABLE_BLUETOOTH and ENABLE_LTE :
    ...
    elif ENABLE_WIFI and not ENABLE_LTE and ENABLE_BLUETOOTH:
        if tracked_inter_users_wifi_dict[user_id] == ble_ids:
            # ml.logger.info(user_id, tracked_inter_users_wifi_dict[user_id])
            wifi_inter__single_users.add(user_id)
        else:
            wifi_inter__multiple_users.add(user_id) 
             
        if tracked_inter_users_wifi_dict[user_id] == wifi_ids:
            # ml.logger.info(user_id, tracked_inter_users_wifi_dict[user_id])
            ble_inter__single_users.add(user_id)
        else:
            ble_inter__multiple_users.add(user_id)   
        
    elif ENABLE_LTE and not ENABLE_WIFI and ENABLE_BLUETOOTH:
...
ml.logger.info(f"LTE: {ENABLE_LTE}, WIFI: {ENABLE_WIFI}, BLE: {ENABLE_BLUETOOTH}")

if ENABLE_LTE:
    ml.logger.info(f"Total LTE Inter users with single mapping (set having same intra mappings):  {len(lte_inter__single_users)}")
    ml.logger.info(f"Total LTE Inter users with multiple mapping  (set having different intra mappings):  {len(lte_inter__multiple_users)}")
    ml.logger.info(f"Total LTE Inter tracked users: {len(tracked_inter_lte_users)}")

elif ENABLE_WIFI:
    ml.logger.info(f"Total Wifi Inter users with single mapping: {len(wifi_inter__single_users)}")
    ml.logger.info(f"Total Wifi Inter users with multiple mapping: {len(wifi_inter__multiple_users)}")
    ml.logger.info(f"Total Wifi Inter tracked users: {len(tracked_inter_wifi_users)}")

elif ENABLE_BLUETOOTH:
    ml.logger.info(f"Total BLE Inter users with single mapping: {len(ble_inter__single_users)}")
    ml.logger.info(f"Total BLE Inter users with multiple mapping: {len(ble_inter__multiple_users)}")
    ml.logger.info(f"Total BLE Inter tracked users: {len(tracked_inter_ble_users)}")
