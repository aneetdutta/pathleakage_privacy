...
md = MongoDB()
ml = MyLogger("tracking_single_257")
...
md.db['baseline_intra_mappings_257'].drop()
md.db['visited_baseline_intra_list_257'].drop()

for i, j in intra_potential_mapping_list.items():
    result = md.db['baseline_intra_mappings_257'].update_one(
            {"_id": str(i)},
...
for i, j in visited_intra_mapping_list.items():
    result = md.db['visited_baseline_intra_list_257'].update_one(
            {"_id": str(i)},
            {"$set": {"_id": str(i), "mapping": list(j)}},
            upsert=True  # Create a new document if no document matches the filter
        )