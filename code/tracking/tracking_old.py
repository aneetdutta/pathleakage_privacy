...
from services.tracking_algorithm_old import tracking_algorithm_old
...
for timestep_pair in timestep_pairs:
    ...
    intra_potential_mapping, inter_potential_mapping, visited_inter_list, visited_intra_list  =  tracking_algorithm_old(two_timestep_data, intra_potential_mapping=intra_potential_mapping, inter_potential_mapping=inter_potential_mapping, visited_inter_list=visited_inter_list, visited_intra_list=visited_intra_list)
...
md.db['intra_mappings_old'].drop()
md.db['inter_mappings_old'].drop()
md.db['visited_inter_list_old'].drop()
md.db['visited_intra_list_old'].drop()

for i, j in intra_potential_mapping_list.items():
    result = md.db['intra_mappings_old'].update_one(
            {"_id": str(i)},
...
for i, j in inter_potential_mapping_list.items():
    result = md.db['inter_mappings_old'].update_one(
            {"_id": str(i)},
            {"$set": {"_id": str(i), "mapping": list(j)}},
            upsert=True  # Create a new document if no document matches the filter
        )
...
for i, j in visited_inter_mapping_list.items():
    result = md.db['visited_inter_list_old'].update_one(
            {"_id": str(i)},
            {"$set": {"_id": str(i), "mapping": list(j)}},
            upsert=True  # Create a new document if no document matches the filter
        )
for i, j in visited_intra_mapping_list.items():
    result = md.db['visited_intra_list_old'].update_one(
            {"_id": str(i)},
            {"$set": {"_id": str(i), "mapping": list(j)}},
            upsert=True  # Create a new document if no document matches the filter
        )