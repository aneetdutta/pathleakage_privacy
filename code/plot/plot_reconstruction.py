...
if ENABLE_BLUETOOTH:
    baseline_ble_df = pd.read_csv(f'csv/baseline_ble_{DB_NAME}.csv')
...
if ENABLE_SMART_TRACKING:
    if ENABLE_WIFI:
        ...
        plt.plot(baseline_smart_wifi_users, baseline_smart_wifi_scores_sorted, label='Baseline & Localization (Wifi)', alpha=0.7, linewidth=2, color="#0072B2")
    if ENABLE_LTE:
        ...
        plt.plot(baseline_smart_lte_users, baseline_smart_lte_scores_sorted, label='Baseline & Localization (LTE)', alpha=0.7, linewidth=2, color="#D55E00")

    if ENABLE_BLUETOOTH:
        baseline_smart_ble_df = pd.read_csv(f'csv/baseline_smart_ble_{DB_NAME}.csv')
        ...
        plt.plot(baseline_smart_ble_users, baseline_smart_ble_scores_sorted, label='Baseline & Localization (Bluetooth)', alpha=0.7, linewidth=2, color="#CC79A7")
