...
with open("data/ri.json") as f:
    json_data = json.load(f)
    
total_devices = 1#len(list(json_data))
total_items = 5
...
for device, data in json_data.items():
    ...
    for d, item_ in data.items():
        if d == "wifi_disconnected":
            data_by_device[i][2] = item_["randomization_interval"] 
        # print(i, d)
        # print(data_by_device)
    i=i+1
...
ax.set_ylabel('randomization Interval', fontsize=14)
ax.set_title('randomization Interval for each device', fontsize=16)
...
xticks = [2.5]  # Centered positions for each device
...
# Improve layout
plt.tight_layout()
plt.savefig('images/plot_randomization_interval.pdf', dpi=600)
# Show plot
plt.show()
