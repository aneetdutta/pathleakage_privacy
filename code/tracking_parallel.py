...
class DeviceManager:
    ...
    def create_device(self, bluetooth_id, wifi_id, lte_id):
        ...
        for device in self.device_list:
            if (
                getattr(device, "bluetooth_id") is not None
                and getattr(device, "bluetooth_id") == bluetooth_id
            ):

                flag = 1
            if (
                getattr(device, "wifi_id") is not None
                and getattr(device, "wifi_id") == wifi_id
            ):

                flag1 = 1
            if (
                getattr(device, "lte_id") is not None
                and getattr(device, "lte_id") == lte_id
            ):
...
def rule_4(manager, line1, line2):
    ...
    if len(d) == 0:
        if (
            len(d1) == 1
            and len(d2) == 1
            and len(sc_1) == len(sb_1)
            and len(sb_2) == len(sc_2)
        ):
            mapping = (d1[0], d2[0])
        ...
        if (
            ...
            and len(sc_1) == len(sb_1)
            and len(sb_2) == len(sc_2)
            and len(sa_1) == len(sb_1)
            and len(sa_1) == len(sc_1)
            and len(sa_2) == len(sb_2)
            and len(sa_2) == len(sc_2)
        ):
...
def calculate_distance(line1, line2):
    ...
    # print(line2)
    return (
        (line1["location"][0] - line2["location"][0]) ** 2
        + (line1["location"][1] - line2["location"][1]) ** 2
    ) ** 0.5
...
def process_target_time(target_time):
    if target_time in D.keys():
        l = D[target_time]
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        # Map the process_target_time function to the range of target times
        pool.map(process_target_time, range(0, 7199))
